...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        mask_path = osp.join(self.masks_dir, img_name.replace('.jpg', '.bmp'))
        ...
        n, indices = np.unique(mask, return_inverse=True)
        mask = Image.fromarray(indices.reshape(mask.size).astype(np.uint8))
